...
function fakeCluster(): {
  ...
  const transport: KubernetesHttpTransport = async (request) => {
    ...
    if (request.method === "POST") {
      ...
      const prefix = kind === "Job" ? "/apis/batch/v1" : kind === "Namespace" ? "/api/v1" : kind === "Secret" ? "/api/v1" : "/api/v1";
      const plural: Record<string, string> = { Namespace: "namespaces", ResourceQuota: "resourcequotas", LimitRange: "limitranges", PersistentVolumeClaim: "persistentvolumeclaims", ServiceAccount: "serviceaccounts", Secret: "secrets", Job: "jobs", Deployment: "deployments", Service: "services" };
